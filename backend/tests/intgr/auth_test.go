...
import (
	...
	"backend/internal/repo/postgres/user"
)
...
func (suite *AuthSuite) TestSignUpSuccess_01(t provider.T) {
	...
	t.WithNewStep("successfully signup new user", func(sCtx provider.StepCtx) {
		...
		// logger := logger.New("info", loggerFile)
		r := postgres.NewRetryAdapter(dbConnector, 2, 2)

		userStorage := user.NewUserRepo(mockLogger, r)
		pasetoStruct, _ := paseto.NewPaseto(paseto.KEY)

		authService := auth.NewAuthService(pasetoStruct, mockLogger, userStorage)

		req := &serviceDto.SignUpRequest{
		...
		// mockUserStorage.On("Add", ctx, mock.Anything).Return(&repoDto.AddUserResponse{}, nil).Run(
		// 	func(args mock.Arguments) {
		// 		r := args.Get(1).(*repoDto.AddUserRequest)
